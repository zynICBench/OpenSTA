                          ...
                          int line)
{
  ...
  VerilogModule *module = new VerilogModule(name, ports, stmts, filename_, line, this);
                             ...
                             VerilogNetSeq *ports)
{
  ...
  for (VerilogNet *mod_port : *ports) {
    ...
    if (port_names.findKey(port_name) == nullptr) {
      ...
      warn(165, module->filename(), module->line(), "module %s repeated port name %s.", module->name(), port_name);
  }
                            ...
                            const char *port_name)
{
  ...
  if (dcl) {
    ...
    Port *port = dcl->isBus()
        ? network_->makeBusPort(cell, port_name, dcl_bus->fromIndex(), dcl_bus->toIndex())
        : network_->makePort(cell, port_name);
    network_->setDirection(port, dir);
    return port;
  }
  else {
    warn(166, module->filename(), module->line(), "module %s missing declaration for port %s.", module->name(), port_name);
                               ...
                               StringSet &port_names)
{
  ...
  while (dcl_iter.hasNext()) {
    ...
    if (dir->isInput() || dir->isOutput() || dir->isBidirect()) {
      if (port_names.findKey(port_name) == nullptr)
        linkWarn(197, module->filename(), module->line(), "module %s declared signal %s is not in the port list.", module->name(), port_name);
    }
  }
}
                              ...
                              const int line)
{
  ...
  if (liberty_cell && hasScalarNamedPortRefs(liberty_cell, pins)) {
    ...
    for (VerilogNet *vnet : *pins) {
      ...
      if (prev_net_name && prev_net_name != unconnected_net_name)
    ...
    VerilogInst *inst = new VerilogLibertyInst(liberty_cell, inst_name, net_names, line);
...
VerilogReader::reportStmtCounts()
{
  if (debug_->check("verilog", 1)) {
    ...
    printClassMemory("port ref scalar", VerilogNetPortRefScalar, net_port_ref_scalar_count_);
    printClassMemory("port ref scalar net", VerilogNetPortRefScalarNet, net_port_ref_scalar_net_count_);
    printClassMemory("port ref bit", VerilogNetPortRefBit, net_port_ref_bit_count_);
    printClassMemory("port ref part", VerilogNetPortRefPart, net_port_ref_part_count_);
  ...
  for (VerilogDclArg *arg : *dcl->args()) {
    ...
    if (net_name) {
      ...
      if (existing_dcl) {
        ...
        else if (dcl->direction()->isPowerGround() && (existing_dir->isOutput() || existing_dir->isInput() || existing_dir->isBidirect()))
          // supply0/supply1 dcl can be used as modifier for
          // input/output/inout dcls.
          dcl_map_[net_name] = dcl;
        else if (!dcl->direction()->isInternal())
          reader->warn(18, filename_, dcl->line(), "signal %s previously declared on line %d.", reader->netVerilogName(net_name), existing_dcl->line());
      }
      else
        dcl_map_[net_name] = dcl;
    }
                                 ...
                                 VerilogReader *reader)
{
  ...
  if (inst_names.findKey(inst_name)) {
    ...
    reader->warn(19, filename_, inst->line(), "instance name %s duplicated - renamed to %s.", reader->instanceVerilogName(inst_name), replacement_name);
...
VerilogLibertyInst::~VerilogLibertyInst()
{
  ...
  for (int i = 0; i < port_count; i++) {
    ...
    if (net_name && net_name != unconnected_net_name)
...
{
  return (to_index_ > from_index_ && index_ <= to_index_) || (to_index_ <= from_index_ && index_ >= to_index_);
}
...
{
  return (net_name_iter_ && net_name_iter_->hasNext()) || net_iter_.hasNext();
}
                             ...
                             VerilogModule *module)
{
  if (name) {
    ...
    if (dcl && dcl->isBus()) {
      VerilogDclBus *dcl_bus = dynamic_cast<VerilogDclBus *>(dcl);
      return new VerilogBusNetNameIterator(name, dcl_bus->fromIndex(), dcl_bus->toIndex());
    }
    else
                                    ...
                                    VerilogReader *reader)
{
  ...
  *t = '\0';
  ...
  if (length > max_length || (length == max_length && strcmp(tmp, reader->constant10Max()) > 0))
    reader->warn(20, reader->filename(), reader->line(), "base 10 constant greater than %s not supported.", reader->constant10Max());
  else {
    char *end;
...
public:
  ...
  void findSource(std::string const & path) const {    
    //split path
    if(path.find("/") != -1){
      int pos = path.find("/");
      std::string inst = path.substr(0, pos);
      std::cout << pos << std::endl; 
      std::cout << inst << std::endl; 
      findSource(path.substr(pos+1));
    }
...
VerilogReader::getMap(VerilogModule *module)
{
  ...
  for (auto &s : *module->ports()) {
    ...
    // std::cout << "module: " << module->name() << "  --------> port: " << s->name() << std::endl; 
    //umap_module_to_port.insert({module->name(), s->name()});
  }
  ...
  for (auto &s : *module->stmts()) {
    ...
    if (s->isModuleInst()) {
      ...
      // std::cout << "module: " << module->name() << "  --------> inst: " << inst_name << std::endl;
      // std::cout << "module inst: " << inst_name << "  --------> module: " << module_name << std::endl;
      // umap_module_to_inst.insert({module->name(), inst_name});
      // umap_inst_to_module.insert({inst_name, module_name});

      // auto iter = umap_inst_to_module.find(inst_name);
      ...
      //    ((VerilogNetPortRefScalarNet *)net)->
    ...
    //     // std::cout << item->pinIndex() << ": port: " << port << std::endl;
    //     if (netNames[item->pinIndex()]) {
    //       const char *pin = netNames[item->pinIndex()];
    //       // std::cout << "net: " << pin << std::endl;
    //       std::unordered_map<const char *, const char *> pair;
    //       pair.insert({pin, port});
    //       // std::cout << "liberty inst: " << inst_name << "  --------> pin: " << port << " port: " << pin << std::endl;
    //       umap_inst_to_net.insert({inst_name, pair});
    //     }
    //   }
    }
  }
...
{
  // std::string origin = "u_cm3_sync_dbg_en/d_async_i";
  // const char *origin_inst = "u_cm3_sync_dbg_en";
  // const char *origin_pin = "d_async_i";

  std::string origin = "u_cm3_dap_ahb_ap/dap_ahb_habort_o";
  const char *origin_inst = "u_cm3_dap_ahb_ap";
  const char *origin_pin = "dap_ahb_habort_o";

  for (auto &s : *module->stmts()) {
    if (s->isModuleInst()) {
      module_cell(s, "u_cm3_dap_ahb_ap", origin_inst, origin_pin);
    }
  }
}

void
VerilogReader::module_cell(VerilogStmt *s, std::string result, const char *origin_inst, const char *origin_pin)
{
  // find instance and module
  // if (std::strcmp(((VerilogModuleInst *)s)->instanceName(), origin_inst) == 0) {
  std::cout << "get inst: " << ((VerilogModuleInst *)s)->instanceName() << std::endl;
  result += std::string("/") + ((VerilogModuleInst *)s)->instanceName();
  const char *module_name = ((VerilogModuleInst *)s)->moduleName();
  VerilogNetSeq *seq = ((VerilogModuleInst *)s)->pins();
  for (auto &s : *seq) {
    std::cout << "port name : " << s->name() << std::endl;
    if (std::strcmp(s->name(), origin_pin) == 0) {
      std::cout << "get module: " << module_name << std::endl;
      Cell *cell = network_->findAnyCell(module_name);
      VerilogModule *module2 = this->module(cell);

      for (auto &s1 : *module2->stmts()) {
        if (s1->isModuleInst()) {
          std::cout << "this is module_cell: " << ((VerilogModuleInst *)s1)->instanceName() << std::endl;
          module_cell(s1, result, origin_inst, origin_pin);
        }
        else if (s1->isLibertyInst()) {
          std::cout << "this is liberty_cell: " << ((VerilogModuleInst *)s1)->instanceName() << std::endl;
          liberty_cell(s1, result, origin_inst, origin_pin);
        }
        // }
      }
      break;
...
void
VerilogReader::liberty_cell(VerilogStmt *s, std::string result, const char *origin_inst, const char *origin_pin)
{
  // source pin in the instance
  std::cout << "start to source pin in the instance ... " << std::endl;

  std::string inst_name = ((VerilogModuleInst *)s)->instanceName();
  std::cout << "source inst: " << inst_name << std::endl;
  ...
  while (iter->hasNext()) {
    auto item = iter->next();
    std::string port = item->name();
    std::cout << item->pinIndex() << ": port: " << port << std::endl;
    if (netNames[item->pinIndex()]) {
      std::cout << "net: " << netNames[item->pinIndex()] << std::endl;
      if (std::strcmp(netNames[item->pinIndex()], origin_pin) == 0) {
        std::cout << "get target net: " << netNames[item->pinIndex()] << std::endl;
        result += "/" + inst_name + "/" + port;
        std::cout << std::endl;
        std::cout << origin_inst << "/" << origin_pin << " ------> " << result << std::endl;
        return;
      }
    }
  }
                           ...
                           Report *report)
{
  if (library_) {
    ...
    if (module) {
      ...
      modulelist.findSource("Adolf/Hitler/Joserf/Starlin/o");
      // symbolTable(module);
      // getMap(module);
      // std::cout << "total size: " << umap_inst_to_module.size() << std::endl;
      // namemap(module);
                                     ...
                                     bool make_black_boxes)
{
  ...
  if (cell == nullptr) {
    if (make_black_boxes) {
      ...
      linkWarn(198, filename_, mod_inst->line(), "module %s not found. Creating black box for %s.", mod_inst->moduleName(), verilogName(mod_inst));
    }
    else
      linkError(199, filename_, mod_inst->line(), "module %s not found for instance %s.", mod_inst->moduleName(), verilogName(mod_inst));
  }
  if (cell) {
    LibertyCell *lib_cell = network_->libertyCell(cell);
    if (lib_cell)
      cell = network_->cell(lib_cell);
    Instance *inst = network_->makeInstance(cell, mod_inst->instanceName(), parent);
    if (lib_cell) {
    ...
    if (mod_inst->hasPins()) {
      if (mod_inst->namedPins())
        makeNamedInstPins(cell, inst, mod_inst, &bindings, parent, parent_module, parent_bindings, is_leaf);
      else
        makeOrderedInstPins(cell, inst, mod_inst, &bindings, parent, parent_module, parent_bindings, is_leaf);
    }
    if (!is_leaf) {
  ...
  while (pin_iter.hasNext()) {
    ...
    if (port) {
      if (vpin->hasNet() && network_->size(port) != vpin->size(parent_module))
        linkWarn(200, parent_module->filename(), mod_inst->line(), "instance %s port %s size %d does not match net size %d.", verilogName(mod_inst), network_->name(port), network_->size(port), vpin->size(parent_module));
      else {
        ...
        if (network_->hasMembers(port)) {
          ...
          while (port_iter->hasNext()) {
            ...
            makeInstPin(inst, port, net_name_iter, bindings, parent, parent_bindings, is_leaf);
          }
          delete port_iter;
        }
        else {
      ...
      if (pg_port == nullptr)
        linkWarn(201, parent_module->filename(), mod_inst->line(), "instance %s port %s not found.", verilogName(mod_inst), port_name);
    }
  }
}
                                   ...
                                   bool is_leaf)
{
  ...
  while (pin_iter.hasNext() && port_iter->hasNext()) {
    ...
    if (network_->size(port) != net->size(parent_module))
      linkWarn(202, parent_module->filename(), mod_inst->line(), "instance %s port %s size %d does not match net size %d.", verilogName(mod_inst), network_->name(port), network_->size(port), net->size(parent_module));
    else {
                               ...
                               VerilogBindingTbl *parent_bindings)
{
  ...
  Instance *inst = network_->makeInstance(cell, lib_inst->instanceName(), parent);
...
{
  const char *module_name = mod_inst->moduleName();
  Cell *cell = network_->makeCell(library_, module_name, true, parent_module->filename());
  if (mod_inst->namedPins())
                              ...
                              VerilogBindingTbl *bindings)
{
  ...
  if (lhs->size(module) == rhs->size(module)) {
    ...
    linkWarn(203, module->filename(), assign->line(), "assign left hand side size %d not equal right hand size %d.", lhs->size(module), rhs->size(module));
}
...
{
  sta::verilog_reader->report()->fileError(164, sta::verilog_reader->filename(), sta::verilog_reader->line(), "%s", msg);
  verilogFlushBuffer();
  return 0;
}
